...
def main():
    # 1 - большие коллекции с конфигами (1500+)
    # 2 - коллекции поменьше
    url_map = {
        '0': "your set of links",
        ...
        '3': "https://raw.githubusercontent.com/RaitonRed/ConfigsHub/main/Splitted-By-Protocol/vless.txt https://github.com/igareck/vpn-configs-for-russia/raw/refs/heads/main/Vless-Reality-Black-Lists-Rus-Cable.txt https://github.com/igareck/vpn-configs-for-russia/raw/refs/heads/main/Vless-Reality-Black-Lists-Rus-Mobile.txt https://github.com/igareck/vpn-configs-for-russia/raw/refs/heads/main/Vless-Reality-White-Lists-Rus-Cable.txt https://github.com/igareck/vpn-configs-for-russia/raw/refs/heads/main/Vless-Reality-White-Lists-Rus-Mobile.txt https://github.com/Freedom-Guard-Builder/Freedom-Finder/raw/refs/heads/main/out/raw_all.txt https://raw.githubusercontent.com/4n0nymou3/multi-proxy-config-fetcher/refs/heads/main/configs/proxy_configs.txt https://raw.githubusercontent.com/LalatinaHub/Mineral/refs/heads/master/result/nodes https://raw.githubusercontent.com/zieng2/wl/refs/heads/main/vless_lite.txt",
        '4': "https://raw.githubusercontent.com/LalatinaHub/Mineral/refs/heads/master/result/nodes",
    }
    ...
    # Для a писать 'ss://'
    # Для b писать 'security=reality flow=xtls-rprx-vision'
    # Фильтры
    ...
    # Чтение all.txt для восстановления чистых конфигов (убираем старые хвосты)
    if os.path.exists('all.txt'):
        with open('all.txt', encoding='utf-8') as f:
            for line in f:
                ...
                # Если строка содержит наш суффикс, обрезаем всё после последнего #
                if '@HydranetVPN🐉' in line:
                    # Ищем последний #, так как в самом конфиге тоже могут быть # (редко, но бывает в параметрах)
                    # Но обычно формат vless://...#name. Мы заменяем name.
                    # Надежнее найти позицию перед #<цифра>
                    match = re.search(r'#\d+[\U0001F1E6-\U0001F1FF]*@HydranetVPN🐉$', line)
                    if match:
                        # Обрезаем строку до начала этого совпадения
                        cleaned = line[:match.start()]
                        existing_lines.add(cleaned)
                        continue
                
                # Старый формат или чистый конфиг (обрезаем комментарии старого типа)
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for result in executor.map(fetch, urls):
            for line in result:
                # Очищаем от комментариев источника
                cleaned = line.split('#')[0].strip()
    ...
    print(f"Processing {total_lines} lines for flags...")

    def get_flag(code):
        if not code: return ''
        # Преобразуем код страны (например, 'US') во флаг (🇺🇸)
    ...
    # Словарь для кэширования стран по IP
    ip_country = {}
    unique_ips = set()

    # 1. Сбор уникальных IP
    for line in sorted_lines:
        # Регулярка ищет IP после @ и перед : (порт)
        # Поддерживает vless://uuid@IP:PORT... и ss://...@IP:PORT...
        match = re.search(r'@([^:@]+):(\d+)', line)
        if match:
            ip = match.group(1)
            unique_ips.add(ip)

    # 2. Запрос гео-данных пакетно
    if unique_ips:
        ips_list = list(unique_ips)
        batch_size = 100
        total_batches = (len(ips_list) - 1) // batch_size + 1
        
        for i in range(0, len(ips_list), batch_size):
            batch_ips = ips_list[i:i + batch_size]
            current_batch = i // batch_size + 1
            print(f"Resolving IPs batch {current_batch}/{total_batches}...")
            batch_results = get_country_batch(batch_ips)
            ip_country.update(batch_results)

    # 3. Формирование финального списка
    print("Formatting output...")
    for i, line in enumerate(sorted_lines, 1):
        # i - номер (1, 2, 3...) без ведущих нулей
        
        flag = ''
        # Извлекаем IP снова для получения флага
        match = re.search(r'@([^:@]+):', line)
        if match:
            ip = match.group(1)
            country_code = ip_country.get(ip, '')
            flag = get_flag(country_code)
        
        # Формируем суффикс: #<номер><флаг>@HydranetVPN🐉
        suffix = f"#{i}{flag}@HydranetVPN🐉"
        
        # Склеиваем чистый конфиг и суффикс
        final_line = f"{line}{suffix}"
        processed.append(final_line)

    # Запись результата
    with open('all.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(processed))

    print(f"Done. Added {added} new lines. Total: {total_lines}")
    print("Output format: <config>#<num><flag>@HydranetVPN🐉")
    ...
    if update_old:
        ...
        with open('old.json', 'w', encoding='utf-8') as f:
            json.dump(list(combined_data), f, indent=2, ensure_ascii=False)
